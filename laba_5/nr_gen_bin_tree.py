from math import log2


def gen_bin_tree_mas(start: int,
                     lvl: int,
                     go_left=lambda n: 2 * (n + 1),
                     go_right=lambda n: 2 * (n - 1)):
    '''функция образующая бинарное дерево в массиве
    в котором элементы записаны по уровням слева на право

    >> gen_bin_tree_mas(4, 3)

    >> [4, 10, 6, 22, 18, 14, 10]
    '''
    tree = [start]
    if lvl < 0:
        raise ValueError('Ошибка! Введен отрицательный уровень!')
    for i in range(lvl - 1):
        for j in range((2**i) - 1, len(tree)):
            tree.append(go_left(tree[j]))
            tree.append(go_right(tree[j]))
    return tree


def sp_tree(a):
    '''функция преобразующая массив из gen_bin_tree_mas()
    в словарь установленного образца

    >> tree = [4, 10, 6, 22, 18, 14, 10]

    >> sp_tree(tree)

    >> {'4': [{'10': [{'22': []}, {'18': []}]}, {'6': [{'14': []}, {'10': []}]}]}
    '''
    lvl = int(log2(len(a) + 1))
    if log2(len(a) + 1) % 1 != 0:
        raise ValueError('Ошибка!')
    m = [[] for i in range(2**lvl)]
    for i in range(lvl - 1):
        lvl -= 1
        l = m
        m = []
        k = 0
        for x in a[len(a) - (2**lvl):]:
            m.append({f'{x}': l[k]})
            k += 1
        for j in range(len(m)):
            a.pop(-1)
        if len(m) > 2:
            m = [[m[i], m[i + 1]] for i in range(0, len(m) - 1, 2)]

    return {f'{a[0]}': m}


print(gen_bin_tree_mas(4, 3))
print(sp_tree(gen_bin_tree_mas(15, 6)))

...
def logger(func: Optional[Callable] = None, *, handle: Any = sys.stdout) -> Callable:
    """
            ...
            try:
                ...
                if is_logger:
                    if "Дискриминант отрицательный" in str(e):
                        handle.warning(error_message)  # WARNING для отрицательного дискриминанта
                    elif "Оба коэффициента" in str(e):
                        handle.critical(error_message)  # CRITICAL для полностью невозможной ситуации
                    else:
